...
followup_2026-05-04: First overlay shipped surfaced a second bug — `Sheet.tsx` autofocuses the first focusable child on open, which (with empty reminders) was the datetime-local input. iOS Safari opens the native picker on programmatic `.focus()`, so the picker popped on sheet mount and stayed focused — making subsequent taps no-ops. Fixed by reordering the DOM so the Anuluj button precedes the input, with flex `order` preserving the visual layout.
verification: Unit tests green (373/373). Vitest, typecheck, build all pass. Verified end-to-end via Playwright (iPhone UA, 390×844, touch): on sheet open, `document.activeElement` is the Anuluj button (not the input); visible layout still has Dodaj termin above Anuluj; `document.elementFromPoint` at the affordance centre returns the datetime-local input; tap focuses it (`activeElement.type === 'datetime-local'`); zero console errors.
files_changed:
